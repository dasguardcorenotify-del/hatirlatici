...
class PackagingReleaseTests(unittest.TestCase):
    ...
    def test_source_build_policy(self):
        ...
        self.assertNotIn(".ci-venv", quality)
        self.assertNotIn("${{ runner.temp }}", quality)
        self.assertIn('temporary = Path(os.environ["RUNNER_TEMP"]).resolve()', quality)
        self.assertIn('print(f"CI_VENV={environment}", file=target)', quality)
        self.assertIn('echo "$CI_VENV/bin" >> "$GITHUB_PATH"', quality)
        self.assertIn('if temporary not in environment.parents:', quality)
